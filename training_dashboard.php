...
if(isset($_POST['submit_course'])) {
    ...
    // Admin email body
    $admin_body = "
        <h3>New Course Posted for Approval</h3>
        ...
        <hr>
        <p>Please review and approve this course from the admin panel.</p>
        <br>
    ...
    $center_body = "
        ...
        <hr>
        <p>You will be notified once the admin approves your course.</p>
        <br>
    ...
    $_SESSION['success_message'] =
        "✅ Course posted successfully! Awaiting admin approval. Confirmation emails sent.";
...
<head>
    ...
    .stat-card.total-courses { border-left-color: var(--secondary-color); }
    .stat-card.active-courses { border-left-color: var(--success-color); }
    .stat-card.pending-courses { border-left-color: var(--warning-color); }
    .stat-card.total-applications { border-left-color: var(--info-color); }
    .stat-card.today-applications { border-left-color: var(--primary-color); }
    ...
    .stat-icon.total-courses { background: rgba(139, 92, 246, 0.1); color: var(--secondary-color); }
    .stat-icon.active-courses { background: rgba(16, 185, 129, 0.1); color: var(--success-color); }
    .stat-icon.pending-courses { background: rgba(245, 158, 11, 0.1); color: var(--warning-color); }
    .stat-icon.total-applications { background: rgba(59, 130, 246, 0.1); color: var(--info-color); }
    .stat-icon.today-applications { background: rgba(0, 160, 152, 0.1); color: var(--primary-color); }
    ...
    .action-card.post-course { border-left-color: var(--secondary-color); }
    .action-card.view-courses { border-left-color: var(--success-color); }
    .action-card.manage-profile { border-left-color: var(--warning-color); }
    .action-card.view-applications { border-left-color: var(--info-color); }
    ...
    .application-pending { background: #fef3c7; color: #92400e; }
    .application-approved { background: #d1fae5; color: #065f46; }
    .application-rejected { background: #fee2e2; color: #991b1b; }
    </style>
</head>
<body>
        ...
        <div class="topbar">
            ...
            <div>
                <span class="badge bg-success rounded-pill px-3 py-2">
                    <i class="fas fa-graduation-cap me-1"></i> Training Center Account
                </span>
            </div>
        </div>
        ...
        <div class="content-wrapper">
            <?php if($active_tab == 'training_dashboard'): ?>
                ...
                <div class="quick-actions mb-4">
                    <a href="training_dashboard.php?tab=post_course" class="action-card post-course">
                            ...
                            <?php if($recent_applications->num_rows > 0): ?>
                                <div class="table-responsive">
                                    <table class="table table-modern">
                                        ...
                                        <tbody>
                                            <?php $i=1; while($app = $recent_applications->fetch_assoc()): ?>
                                            <tr>
                                                <td>
                                                    <div class="d-flex align-items-center">
                                                ...
                                                <td>
                                                    <?php 
                                                    $status_class = 'application-pending';
                                                    $status_text = 'Pending';
                                                    if($app['status'] == 'approved') {
                                                        $status_class = 'application-approved';
                                                        $status_text = 'Approved';
                                                    } elseif($app['status'] == 'rejected') {
                                                        $status_class = 'application-rejected';
                                                        $status_text = 'Rejected';
                                                    }
                                                    ?>
                                                    <span class="application-status-badge <?= $status_class ?>">
                                                        <?= $status_text ?>
                                                    </span>
                                                </td>
                                                <td>
...
<div class="card shadow-lg border-0">
    ...
    <div class="card-body p-4">
        ...
        <form method="post">
            <!-- Hidden field for training_center_id will be set in backend -->
                                        ...
                                        <tr>
                                            ...
                                            <td>
                                                <div class="btn-group" role="group">
                                                    <button class="btn btn-sm btn-outline-modern" data-bs-toggle="modal" data-bs-target="#appDetailModal<?= $app['id'] ?>">
                                                        <i class="fas fa-eye"></i>
                                                    </button>
                                                    <?php if($app['status'] == 'pending'): ?>
                                                        <button type="button" class="btn btn-sm btn-outline-success" onclick="updateApplicationStatus(<?= $app['id'] ?>, 'approved')">
                                                            <i class="fas fa-check"></i>
                                                        </button>
                                                        <button type="button" class="btn btn-sm btn-outline-danger" onclick="updateApplicationStatus(<?= $app['id'] ?>, 'rejected')">
                                                            <i class="fas fa-times"></i>
                                                        </button>
                                                    <?php endif; ?>
                                                </div>
    ...
    function updateApplicationStatus(applicationId, status) {
        if (confirm('Are you sure you want to update this application status to "' + status + '"?')) {
            ...
            actionInput.name = 'update_application_status';
            actionInput.value = '1';
